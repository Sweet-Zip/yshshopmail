...
const SideBar = ({ isOpen, setIsOpen }: isOpenProps) => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`
          bg-gradient-to-b from-[#1e293b] to-[#0f172a] flex flex-col text-white 
          min-h-[calc(100vh-120px)] max-h-[calc(100vh-120px)] rounded-2xl 
          shadow-2xl border border-slate-700/50
          overflow-hidden
          fixed md:relative top-5 md:top-0 left-5 md:left-0 z-50 transition-all duration-300
      ...
      >
        {/* Close button - Even higher z-index */}
        <AnimatePresence>
          {isOpen && (
            <motion.button
              className="absolute top-4 right-4 p-2 rounded-lg hover:bg-white/10 transition-colors md:hidden z-[60]"
              onClick={() => setIsOpen(false)}
        ...
        <div className="p-4 sm:p-6 border-t border-slate-600/30 mt-auto">
          <div className="flex items-center justify-center gap-3 p-3 sm:p-4 bg-slate-800/50 rounded-2xl border border-slate-600/30 mb-4">
            ...
            <Link href="/login" className="hover:cursor-pointer">
              <button
                className="text-slate-400 hover:text-red-400 transition-colors p-2 hover:bg-red-500/10 rounded-lg flex-shrink-0"
                title="Log out"
